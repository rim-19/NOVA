    ...
    >
      <div className="w-full max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-start">
          <div className="glass-card p-8 rounded-3xl border-gold-glow">
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              {[
                ...
                <div key={field.name} className="flex flex-col gap-2">
                  ...
                  <input
                    ...
                    onFocus={(e) => (e.target.style.borderBottomColor = "rgba(184,149,106,0.5)")}
                    onBlur={(e) => (e.target.style.borderBottomColor = "rgba(125,23,54,0.25)")}
                  />
                </div>
              ))}
              ...
              <button type="submit" disabled={loading || items.length === 0} className="btn-burgundy animate-pulse-glow mt-4 w-full">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
              {items.map((item) => (
                <div key={`${item.id}-${item.size}`} className="flex justify-between items-start pb-4" style={{ borderBottom: "1px solid rgba(125,23,54,0.1)" }}>
                  <div>
                    <p
                      style={{
                        fontFamily: "Cormorant Garamond, serif",
                    ...
                    <p className="text-label mt-1" style={{ color: "rgba(184,149,106,0.4)", fontSize: "0.6rem", letterSpacing: "0.2em" }}>
                      {item.size && `Size ${item.size} · `}Qty {item.quantity}
                    </p>
                  </div>
                  <p style={{ fontFamily: "Cormorant Garamond, serif", fontSize: "1rem", color: "rgba(184,149,106,0.8)" }}>
            ...
            {items.length === 0 && (
              <p style={{ fontFamily: "MonteCarlo, cursive", fontSize: "1.4rem", color: "rgba(245,233,226,0.2)" }}>
                Your cart is empty.
              </p>
            )}
          </div>
